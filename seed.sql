...
('10000003', 'testuser_003', 'invalid:hash', '一路向北', 1, 30, 5, '这个人很懒，什么都没留下~', datetime('now', '-3 days')),
...
-- ===== 帖子3（1小时前，0赞0评论） =====
INSERT INTO posts (user_id, author_name, author_level, title, content, images, video_url, likes_count, comments_count, favorites_count, created_at)
VALUES ((SELECT id FROM users WHERE username='testuser_003'), '一路向北', 1,
'一路向北？强强',
'这么强？！',
...
(SELECT id FROM users WHERE username='testuser_003'), '一路向北', '蛋挞世界无敌最可爱!', 0, 0, datetime('now', '-12 hours')),
((SELECT id FROM posts WHERE title='我不知道，但是'),
